		...
		if ( ! empty( $_POST ) && check_admin_referer( 'bp_share_nonce', 'nonce' ) && current_user_can( 'manage_options' ) ) {
			if ( get_site_option( $option_name ) !== false ) {
				...
				if ( empty( $services ) ) {
					...
					$html_view .= '</div>';
					$html_view .= '</td>';
					$html_view .= '<td class="service_delete bp-share-td"><p class="service_delete_icon" data-bind="' . $service_key . '"><i class="fa fa-times"></i></p></td>';
					$html_view .= '</tr>';
				} else {
					$new_value = array(
						"chb_$service_value"  => 1,
			...
			echo wp_json_encode( $html_view_arr );
		}
		die();
	}
